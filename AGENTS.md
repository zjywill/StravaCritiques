...
- Root Python entry point lives in `ai_gen_comment.py`; it wires LangChain prompts and `ChatOpenAI` for the CLI demo.  
...
python ai_gen_comment.py "示例问题"            # run the LangChain CLI (prompts if no arg)
...
## Testing Guidelines
- Adopt `pytest` for new coverage; mirror the module layout under `tests/` (e.g., `tests/test_ai_gen_comment.py`, `tests/strava/test_app.py`).  
